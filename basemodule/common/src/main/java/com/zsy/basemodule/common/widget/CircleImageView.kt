...
class CircleImageView(context: Context, attrs: AttributeSet) : AppCompatImageView(context,attrs) {
    ...
    private fun initAttrs(attrs: AttributeSet?) {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.YLCircleImageView)
        //  半径
        radius = ta.getDimension(R.styleable.YLCircleImageView_radius, 0f)
        topLeftRadius = ta.getDimension(R.styleable.YLCircleImageView_topLeftRadius, 0f)
        topRightRadius = ta.getDimension(R.styleable.YLCircleImageView_topRightRadius, 0f)
        bottomLeftRadius = ta.getDimension(R.styleable.YLCircleImageView_bottomLeftRadius, 0f)
        bottomRightRadius = ta.getDimension(R.styleable.YLCircleImageView_bottomRightRadius, 0f)
        //  展示类型
        styleType = ta.getInt(R.styleable.YLCircleImageView_scaleType, TOP)
        //  描边
        borderWidth = ta.getDimension(R.styleable.YLCircleImageView_borderWidth, 0f)
        borderSpace = ta.getDimension(R.styleable.YLCircleImageView_borderSpace, 0f)
        borderColor = ta.getColor(R.styleable.YLCircleImageView_borderColor, Color.WHITE)

        //  不常用属性
        topLeftRadius_x = ta.getDimension(R.styleable.YLCircleImageView_topLeftRadius_x, 0f)
        topLeftRadius_y = ta.getDimension(R.styleable.YLCircleImageView_topLeftRadius_y, 0f)
        topRightRadius_x = ta.getDimension(R.styleable.YLCircleImageView_topRightRadius_x, 0f)
        topRightRadius_y = ta.getDimension(R.styleable.YLCircleImageView_topRightRadius_y, 0f)
        bottomLeftRadius_x = ta.getDimension(R.styleable.YLCircleImageView_bottomLeftRadius_x, 0f)
        bottomLeftRadius_y = ta.getDimension(R.styleable.YLCircleImageView_bottomLeftRadius_y, 0f)
        bottomRightRadius_x = ta.getDimension(R.styleable.YLCircleImageView_bottomRightRadius_x, 0f)
        bottomRightRadius_y = ta.getDimension(R.styleable.YLCircleImageView_bottomRightRadius_y, 0f)
        ta.recycle()
        initData()
    }
    ...
    fun initData() {
        initRadius()
        //  判断是否需要调用绘制函数
        circle = borderWidth != 0f || borderSpace != 0f || topLeftRadius_x != 0f || topLeftRadius_y != 0f || topRightRadius_x != 0f || topRightRadius_y != 0f || bottomLeftRadius_x != 0f || bottomLeftRadius_y != 0f || bottomRightRadius_x != 0f || bottomRightRadius_y != 0f
    ...
    private fun initRadius() {
        ...
        bottomRightRadius_x = if (bottomRightRadius_x == 0f) bottomRightRadius else bottomRightRadius_x
        bottomRightRadius_y = if (bottomRightRadius_y == 0f) bottomRightRadius else bottomRightRadius_y
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        //  绘制描边
        if (borderWidth != 0f) {
            val rectF = RectF(paddingLeft.toFloat(), paddingTop.toFloat(), (vw - paddingRight).toFloat(), (vh - paddingBottom).toFloat())
            //  描边会有一半处于框体之外
            val i = borderWidth / 2
            //  移动矩形，以便于描边都处于view内
            rectF.inset(i, i)
        ...
        if (null != drawable && circle) {
            ...
            val src = getSrc(bitmap, rectF.width().toInt(), rectF.height().toInt())
     ...
     * @param offset 半径偏移量
     */
    private fun drawPath(canvas: Canvas, rectF: RectF, paint: Paint?, offset: Float) {
        val path = Path()
